...
import ListaPedidosPendienteEntrega from '../componentes/ListaPedidosPendienteEntrega';
import './styles/Pedidos.scss'
import {useUser, firestore, useFirebaseApp} from 'reactfire';

function PedidosPendienteEntrega(props){
    ...
    const consultarPedidos= async ()=>{
        ...
        .where("Estado","==","PEDIDO A ENTREGAR")
        .onSnapshot(querySnapShot=>{
            if(querySnapShot.empty){
                return;
            }
            ...
            querySnapShot.forEach((PedidoUsuario)=>{
                if(PedidoUsuario.data().Estado === 'PEDIDO A ENTREGAR'){
                    misPedidosId.push({
                        id: PedidoUsuario.id,
    ...
    return(
        <div className="MisPedidosUsuario">
                <h1>PEDIDOS PENDIENTE ENTREGA</h1>
                
            <div className="container-mis-pedidos">
                    {
                       idPedidos &&
                       idPedidos.map(miPedido=>(
                           <ListaPedidosPendienteEntrega 
                           key={miPedido.id} 
                           Id={miPedido.id} 
...
export default PedidosPendienteEntrega;
