...
import ListaPedidosConcluidos from '../componentes/ListaPedidosConcluidos';
import './styles/Pedidos.scss'
import {useUser, firestore, useFirebaseApp} from 'reactfire';

function PedidosCocluidos(props){
    ...
    const consultarPedidos= async ()=>{
        ...
        .onSnapshot(querySnapShot=>{
            ...
            querySnapShot.forEach((PedidoUsuario)=>{
                if(PedidoUsuario.data().Estado === 'ENTREGADO'){
                    misPedidosId.push({
                        id: PedidoUsuario.id,
...
export default PedidosCocluidos;
